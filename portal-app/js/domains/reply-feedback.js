 ...
 * 永続先は vault の会話ログ1本
 *
 * 評価種別は会話ログに併記済み（`**[14:01] ユーザー（close）:**`）。
 ...
 * 遡る日数を 3→30 に増やしても promptGuide の長さは変わらない（実測で確認）。
 *
 * 禁止語
 *
...
window.ReplyFeedback = {
  ...
  /**
   * 集計の土台になるイベント（localStorage はやめた）。
  ...
  loadFromVault(days = VAULT_LOG_DAYS) {
    // 以前のバージョンが localStorage に残した集計を掃除する。
  ...
  async _loadFromVault(days) {
    ...
    // 並列に読む。直列だと日数に比例して起動が遅くなる（実測 30日で11秒）。
